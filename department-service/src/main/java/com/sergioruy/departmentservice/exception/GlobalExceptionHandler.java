...
public class GlobalExceptionHandler {
    ...
    @ExceptionHandler(DepartmentCodeExistException.class)
    public ResponseEntity<ErrorDetails> handleEmailExistException(DepartmentCodeExistException e,
                                                                  WebRequest webRequest) {
        ErrorDetails errorDetails = new ErrorDetails(
                LocalDateTime.now(),
                e.getMessage(),
                webRequest.getDescription(false),
                "DEPARTMENT_CODE_EXIST"
        );
